...
1.  Sort categories by descending frequency, to establishe ranking from most frequent to least frequent.
    1.  Convert `total_counts` into a list of `(category, count)` pairs.
    1.  Sort by count in descending order.
1.  Compute the grand total.
1.  Draw the first-level grouping (Figure 1):
...
1.  Draw the second-level grouping (Figure 2a):
...
1.  Draw the third-level grouping (Figure 2b):
